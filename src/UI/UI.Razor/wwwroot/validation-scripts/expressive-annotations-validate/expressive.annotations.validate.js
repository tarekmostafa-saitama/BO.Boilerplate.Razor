...
(function($, window) {
    ...
    var
        ...
        api = { // to be accesssed from outer scope
            settings: {
                ...
                optimize:
                    true, // if flag is on, requirement expression is not evaluated for empty fields (otherwise, it is evaluated and such an evaluation result is provided to eavalid event)
                enumsAsNumbers:
                    true, // specifies whether values of enum types are internally treated as integral numerics or string identifiers (should be consistent with the way of how input fields values are stored in HTML)
                dependencyTriggers:
                    "change keyup", // a string containing one or more DOM field event types (such as "change", "keyup" or custom event names) for which fields directly dependent on referenced DOM field are validated

                apply:
                    function(options) { // alternative way of settings setup (recommended), crucial to invoke e.g. for new set of dependency triggers to be re-bound
                        function verifySetup() {
                            ...
                            if (!typeHelper.isString(api.settings.dependencyTriggers) &&
                                api.settings.dependencyTriggers !== null &&
                                api.settings.dependencyTriggers !== undefined) {
                        ...
                        function
                            extend(target,
                                source) { // custom implementation over jQuery.extend() because null/undefined merge is needed as well
                        ...
                        $("form").each(function() {
                            $(this).find("input, select, textarea")
                                .off(".expressive.annotations"); // remove all event handlers in the '.expressive.annotations' namespace
            ...
            addValueParser:
                function(name,
                    func) { // provide custom deserialization methods for values of these DOM fields, which are accordingly decorated with ValueParser attribute at the server-side
        ...
        toolchain = {
            ...
            addMethod:
                function(name,
                    func) { // add multiple function signatures to methods object (methods overloading, based only on numbers of arguments)
                    ...
                    this.methods[name] = function() {
                        ...
                        return func.apply(this,
                            arguments); // no exact signature match, most likely variable number of arguments is accepted
                    };
                },
            ...
            initialize: function() {
                this.addMethod("Now",
                    function() { // return milliseconds
                        return Date.now(); // now() is faster than new Date().getTime()
                    });
                this.addMethod("Today",
                    function() { // return milliseconds
                        return new Date(new Date().setHours(0, 0, 0, 0)).getTime();
                    });
                this.addMethod("ToDate",
                    function(dateString) { // return milliseconds
                        return Date.parse(dateString);
                    });
                this.addMethod("Date",
                    function(year, month, day) { // months are 1-based, return milliseconds
                        return new Date(new Date(year, month - 1, day).setFullYear(year)).getTime();
                    });
                this.addMethod("Date",
                    function(year, month, day, hour, minute, second) { // months are 1-based, return milliseconds
                        return new Date(new Date(year, month - 1, day, hour, minute, second).setFullYear(year))
                            .getTime();
                    });
                this.addMethod("TimeSpan",
                    function(days, hours, minutes, seconds) { // return milliseconds
                        return seconds * 1e3 + minutes * 6e4 + hours * 36e5 + days * 864e5;
                    });
                this.addMethod("Length",
                    function(str) {
                ...
                this.addMethod("Concat",
                    function(strA, strB) {
                        return [strA, strB].join("");
                    });
                this.addMethod("Concat",
                    function(strA, strB, strC) {
                        return [strA, strB, strC].join("");
                    });
                this.addMethod("CompareOrdinal",
                    function(strA, strB) {
                        if (strA === strB) {
                            return 0;
                        }
                ...
                this.addMethod("CompareOrdinalIgnoreCase",
                    function(strA, strB) {
                ...
                this.addMethod("StartsWithIgnoreCase",
                    function(str, prefix) {
                ...
                this.addMethod("EndsWithIgnoreCase",
                    function(str, suffix) {
                        str = (str !== null && str !== undefined) ? str.toLowerCase() : null;
                ...
                this.addMethod("IsPhone",
                    function(str) {
                        ...
                        return /^(\+\s?)?((?!\+.*)\(\+?\d+([\s\-\.]?\d+)?\)|\d+)([\s\-\.]?(\(\d+([\s\-\.]?\d+)?\)|\d+))*(\s?(x|ext\.?)\s?\d+)?$/
                            .test(str);
                    });
                this.addMethod("IsUrl",
                    function(str) {
                ...
                this.addMethod("Min",
                    function(values) { // accepts both, array and variable number of arguments
        ...
        typeHelper = {
                ...
                format: function(text, params) {
                    function makeParam(value) {
                        ...
                        value =
                            ...
                            ? value.replace(/\$/g, "$$$$")
                            : value; // escape $ sign for string.replace()
                        return value;
                    }
            ...
            datetime: {
                tryParse: function(value) {
                    ...
                    if (typeHelper.isString(value)) {
                        var millisec =
                            Date.parse(value); // default parsing of string representing an RFC 2822 or ISO 8601 date
            ...
            guid: {
                tryParse: function(value) {
                    ...
                    return {
                        error: true,
                        msg:
                            "Given value was not recognized as a valid guid - guid should contain 32 digits with 4 dashes (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)."
                    };
                }
            },
            enumeration: {
                tryParse: function(value) {
                    return api.settings.enumsAsNumbers
                        ? typeHelper.number.tryParse(value)
                        : typeHelper.string.tryParse(value);
                }
            },
            isTimeSpan: function(value) {
                return /(\-)?(?:(\d*)\.)?(\d+)\:(\d+)(?:\:(\d+)\.?(\d{3})?)?/
                    .test(value); // regex for recognition of .NET style timespan string, taken from moment.js v2.9.0
            },
            ...
            tryParse: function(value, type, field, parser) {
                ...
                if (parser !== null && parser !== undefined) {
                    parseFunc =
                        typeHelper.findValueParser(field,
                            parser); // pointed by attribute custom field-specific parser lookup - highest parsing priority
                    if (!parseFunc.error) {
                ...
                parseFunc =
                    typeHelper.findValueParser(field,
                        type); // custom type-specific parser lookup - secondary parsing priority
                if (!parseFunc.error) {
                    logger.warn(typeHelper.string.format(
                        "Overriden {0} type parsing runs for {1} field. All fields of {0} type are going to be parsed using your value parser. If such a behavior is unintentional, change the name of your value parser to one, which does not indicate at {0} (or any other) type name.",
                        type,
                        field));
                    return parseFunc(value, field);
                }
            ...
            findValueParser: function(field, parser) {
                ...
                return {
                    error: true,
                    msg: typeHelper.string.format(
                        "Custom value parser {0} not found. Consider its registration with ea.addValueParser(), or remove redundant ValueParser attribute from {1} model field.",
                        parser,
                        field)
                };
            }
        ...
        modelHelper = {
            ...
            extractValue: function(form, name, prefix, type, parser) {
                function getValue(element) {
                    ...
                    case "checkbox":
                        if (element.length > 2) {
                            logger.warn(typeHelper.string.format(
                                "DOM field {0} is ambiguous (unless custom value parser is provided).",
                                element.attr("name")));
                        }
                ...
                parsedValue =
                    typeHelper.tryParse(rawValue, type, fieldName, parser); // convert field value to required type
                if (parsedValue !== null && parsedValue !== undefined && parsedValue.error) {
                    throw typeHelper.string.format("DOM field {0} value conversion to {1} failed. {2}",
                        fieldName,
                        type,
                        parsedValue.msg);
                }
            ...
            adjustGivenValue: function(value, element, params) {
                value =
                    ...
                    ? element.checked
                    : value; // special treatment for checkbox, because when unchecked, false value should be retrieved instead of undefined

                var field = element.name.replace(params.prefix, "");
                var parser = params.parsersMap[field];
                if (parser !== null && parser !== undefined) {
                    var parseFunc =
                        typeHelper.findValueParser(element.name,
                            parser); // pointed by attribute custom field-specific parser lookup - highest parsing priority
                    if (!parseFunc.error) {
            ...
            ctxEval: function(exp, ctx) { // evaluates expression in the scope of context object
                return (new
                    Function("expression",
                        "context",
                        "with(context){return eval(expression)}"))(exp,
                    ctx); // function constructor used on purpose (a hack), for 'with' statement not to collide with strict mode, which
        ...
        validationHelper = {
            ...
            validateReferences: function(name, form) {
                ...
                if (referencedFields !== undefined && referencedFields !== null) {
                    logger.dump(typeHelper.string.format("Validation triggered for following {0} dependencies: {1}.",
                        name,
                        referencedFields.join(", ")));
                    i = referencedFields.length;
                    while (i--) {
                        field = $(form)
                            .find(typeHelper.string.format(':input[data-val][name="{0}"]', referencedFields[i]))
            ...
            bindFields:
                function(form,
                    force) { // attach validation handlers to dependency triggers (events) for some form elements
                    if (api.settings.dependencyTriggers !== null &&
                        api.settings.dependencyTriggers !== undefined &&
                        api.settings.dependencyTriggers !== "") {
                        var namespacedEvents = [];
                        $.each(api.settings.dependencyTriggers.split(/\s+/),
                            function(idx, event) {
                                if (/\S/.test(event)) {
                                    namespacedEvents.push(typeHelper.string.format("{0}.expressive.annotations",
                                        event));
                                }
                            });
                        ...
                        $(form).find("input, select, textarea").not(function(idx, element) {
                            ...
                            return !force && bound;
                        }).on(namespacedEvents.join(" "),
                            function(event) {
                                var field = $(this).attr("name");
                                logger.dump(typeHelper.string.format(
                                    "Dependency validation trigger - {0} event, handled.",
                                    event.type));
        ...
        buildAdapter = function(adapter, options) {
            ...
            validationHelper.collectReferences(typeHelper.object.keys(rules.fieldsMap),
        ...
        computeAssertThat = function(value, element, params) {
            value =
                modelHelper.adjustGivenValue(value,
                    element,
                    params); // preprocess given value (here basically we are concerned about determining if such a value is null or not, to determine if the attribute
            ...
            if (value !== undefined &&
                ...
                var model = modelHelper.deserializeObject(params.form,
                    params.fieldsMap,
                    params.constsMap,
                    params.enumsMap,
                    params.parsersMap,
                ...
                logger.dump(typeHelper.string.format(
                    "AssertThat expression of {0} field:\n{1}\nwill be executed within following context (methods hidden):\n{2}",
                    element.name,
                    params.expression,
                    model));
                return modelHelper.ctxEval(params.expression,
                    model); // verify assertion, if not satisfied => notify (return false)
            }
            return true;
        },
        ...
        computeRequiredIf = function(value, element, params) {
            ...
            var exprVal, model;
            var message =
                "RequiredIf expression of {0} field:\n{1}\nwill be executed within following context (methods hidden):\n{2}";
            if (!
                api.settings
                .optimize) { // no optimization - compute requirement condition despite the fact field value may be provided
                model = modelHelper.deserializeObject(params.form,
                    params.fieldsMap,
                    params.constsMap,
                    params.enumsMap,
                    params.parsersMap,
            ...
            if (value === undefined ||
                value === null ||
                value ===
                "" // check if the field value is not set (undefined, null or empty string treated at client as null at server)
                ||
                ...
                logger.dump(typeHelper.string.format(message, element.name, params.expression, model));
                exprVal =
                    modelHelper.ctxEval(params.expression,
                        model); // verify requirement, if satisfied => notify (return false)
                return {
                    valid: !exprVal,
                    condition: exprVal
                };
    ...
    $.each(annotations.split(""),
        function() { // it would be ideal to have exactly as many handlers as there are unique annotations, but the number of annotations isn't known untill DOM is ready
            var adapter = typeHelper.string.format("assertthat{0}", $.trim(this));
            $.validator.unobtrusive.adapters.add(adapter,
                ["expression", "fieldsMap", "constsMap", "enumsMap", "parsersMap", "errFieldsMap"],
                function(options) {
    ...
    $.each(annotations.split(""),
        function() {
            ...
            $.validator.unobtrusive.adapters.add(adapter,
                ["expression", "fieldsMap", "constsMap", "enumsMap", "parsersMap", "errFieldsMap", "allowEmpty"],
                function(options) {
                    buildAdapter(adapter, options);
                });
        });
            ...
            $.validator.addMethod(method,
                function(value, element, params) {
                    try {
                        ...
                        $(element).trigger("eavalid",
                            ["requiredif", result.valid, params.expression, result.condition]);
                        return result.valid;
                    } catch (ex) {
                        logger.fail(ex);
                    }
